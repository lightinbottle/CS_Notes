...
* Redis的底层数据结构
  ...
  <center> <img src="Pics\redis底层数据结构.png"/></center>

* 基本数据类型的底层实现

  * STRING: 编码方式可以是： int、embstr、raw

  * LIST编码方式可以是： ziplist linkedlist

    ziplist压缩表，节省内存空间，存储在连续的内存空间中；适用于对象元素数目不多，每个元素也不大的时候

    linkedlist 链表

  * HASH编码方式： ziplist hashtable 

    ziplist中hash的键值对是按照key1,value1,key2,value2...的顺序存储的，适用于对象元素数目不多并且元素不大的时候

    hashtable的底层实现是 dict字典

  * SET编码方式： intset、hashtable 

    intset是一个有序集合，查找元素的复杂度为O(logN) 

  * ZSET编码方式   一种是ziplist，另一种是skiplist与dict的结合 

    ziplist也是顺序存储，按照排序

    跳跃表加快排序 范围查找；score相同的情况下 按照字典排序
...
在 rehash 期间，每次对字典执行添加、删除、查找或者更新操作时，都会执行一次**渐进式 rehash**。
...
将热点数据放到内存中，设置**内存的最大使用量**以及**淘汰策略**来保证缓存的命中率。
...
Memcached 仅支持字符串类型，而 Redis **支持五种不同**的数据类型，可以更灵活地解决问题。
...
- 在 Redis 中，并不是所有数据都一直存储在内存中，可以将一些很久没用的 value **交换到磁盘**，而 Memcached 的数据则会**一直在内存中**。
- Memcached 将内存分割成特定长度的块来存储数据，以完全解决内存碎片的问题。但是这种方式会使得内存的利用率不高，例如块的大小为 128 bytes，只存储 100 bytes 的数据，那么剩下的 28 bytes 就浪费掉了。

## 备份 同步

Redis支持master-slave的备份 同步，Memcached不支持

## 单线程 多线程

Memcached能使用多核，redis只能使用单核，对于大数据的处理（100k）memcache的性能高于redis，memcached单机的吞吐量大于redis

## 使用场景

有**持久化需求**或者对数据结构和处理有**高级要求（排序）**的应用，选择redis，其他简单的key/value存储，选择memcache

# 六、键的过期时间

Redis 可以为**每个键设置过期时间**，当键过期时，会自动删除该键

对于散列表这种容器，**只能为整个键设置过期时间**（整个散列表），而不能为键里面的单个元素设置过期时间。

# 七、数据淘汰策略

可以设置**内存最大使用量**，当内存使用量超出时，会施行**数据淘汰策略**。
...
作为内存数据库，出于对性能和内存消耗的考虑，Redis 的淘汰算法实际实现上并非针对所有 key，而是**抽样一小部分并且从中选出**被淘汰的 key。
...
将**某个时间点的所有数据**都存放到硬盘上。

可以将快照复制到其它服务器从而创建具有相同数据的服务器副本。

如果系统发生故障，将会**丢失最后一次创建快照之后的数据**。

如果数据量很大，保存**快照的时间会很长**

```
save 900 10  //表示每900s内有10条写入，就产生快照
```
...
使用 AOF 持久化需要设置同步选项，从而确保**写命令什么时候会同步到磁盘文件（这里的磁盘文件是日志文件）**上。这是因为对文件进行写入并不会马上将内容同步到磁盘上，而是先存储到缓冲区，然后由操作系统决定什么时候同步到磁盘。有以下同步选项：
...
- **everysec** 选项比较合适，可以保证系统崩溃时**只会丢失一秒**左右的数据，并且 Redis 每秒执行一次同步对服务器性能几乎没有任何影响；
- no 选项并不能给服务器性能带来多大的提升，而且也会增加系统崩溃时数据丢失的数量。

随着服务器写请求的增多，AOF 文件会越来越大。Redis 提供了一种将 AOF 重写的特性，能够去除 AOF 文件中的冗余写命令。

## Redis重启时加载RDB AOF的顺序
...
Redis重启 加载RDB更快，AOF文件更大，所以AOF不适用于做冷备份

AOF丢失的数据更少，因为当数据量大的时候RDB一次快照的时间很长

RDB每次都要fork一个子进程（内存拷贝）来生成快照文件，如果redis内存数据特别大，可能会导致redis客户端暂停服务

**综合使用RDB和AOF**：用AOF来保证数据不丢失，作为恢复数据的第一选择；用RDB来做不同程度的冷备，在AOF文件都丢失或损坏不可用的时候，可以使用RDB进行快速的数据恢复。 
...
Redis 基于 **Reactor 模式**开发了自己的网络事件处理器，使用 I/O 多路复用程序来同时监听多个套接字，并将到达的事件传送给文件事件分派器，分派器会根据套接字产生的事件类型调用相应的事件处理器。
...
**分片**是将数据划分为多个部分的方法，可以将数据存储到多台机器里面，这种方法在解决某些问题时可以获得线性级别的性能提升。
...
- **服务器分片**：Redis Cluster。路由方式：将请求发送到**任意节点**，接收到请求的节点会将查询请求发送到正确的节点上执行。

注意，分区的粒度是键，对于一些set  list无法进行拆分
...
* redis和nosql mysql的区别

  mysql关系型数据库 表的格式存储  存储结构化的数据

  nosql 存储非结构化的数据，适用于数据的结构不确定的情况，主要包括：键值对存储Redis 列存储Hbase  文档数据库mongodb  图数据库

  redis是nosql中的一种 

* redis是单线程还是多线程  redis为什么用单线程

  redis是单线程实现，内部采用epoll实现**多路复用**单线程处理多个IO连接，一个线程轮询所有绑定的IO，处理就绪的IO

  跟多线程相比的**优势**：

  * 单线程减少了需要切换到内核的上下文切换，单个线程的性能得到充分的利用
  * 单线程不用考虑加锁的问题
  * 单线程实现更容易

  **缺点**

  * 无法充分发挥CPU多核的性能（可以在一个实例上多启动几个redis进程，用类似集群的方式构建）

  **为什么使用单线程**

  * redis都是内存操作，一般来说cpu不是瓶颈，瓶颈是网络io，如果用多线程，线程间的切换反而会产生很大的消耗
...
* 如何保证redis的高可用
  ...
  **Sentinel**（**哨兵**）可以监听集群中的服务器，并在主服务器进入下线状态时，自动从从服务器中选举出新的主服务器。
  ...
  主从复制实现读写分离，主数据库主要进行写操作，而从数据库负责读操作。 

  主从同步分为：全量同步（RDB） 增量同步
...
* redis的使用场景
  ...
  ​	分布式锁是分布式环境中多进程（不同主机上的进程）的同步方式

  session缓存 session cache
...
* 缓存崩溃
  ...
  指的是由于数据没有被加载到缓存中，或者缓存数据在同一时间**大面积失效（过期**），又或者缓存服务器宕机，导致大量的请求都到达数据库。

  在有缓存的系统中，系统非常依赖于缓存，缓存分担了很大一部分的数据请求。当发生缓存雪崩时，数据库无法处理这么大的请求，导致**数据库崩溃**。
...
* 缓存的“二八定律”、
  ...
  *二八定律*:80%的业务访问集中在20%的数据上 ，所以要找最频繁访问的那部分数据
